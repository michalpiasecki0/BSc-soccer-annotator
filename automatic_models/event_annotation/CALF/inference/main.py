...
from automatic_models.event_annotation.CALF.inference.dataset import SoccerNetClipsTesting
from automatic_models.event_annotation.CALF.inference.model import ContextAwareModel
from automatic_models.event_annotation.CALF.inference.train_infer import test
...
def main(args):
    ...
    if args.device == 'cpu':
        model = ContextAwareModel(weights=args.load_weights, input_size=args.num_features,
                                  ...
                                  receptive_field=args.receptive_field*args.framerate,
                                  num_detections=dataset_Test.num_detections, framerate=args.framerate).cpu()
    elif args.devide == 'cuda':
        model = ContextAwareModel(weights=args.load_weights, input_size=args.num_features,
                                  num_classes=dataset_Test.num_classes,
                                  chunk_size=args.chunk_size * args.framerate, dim_capsule=args.dim_capsule,
                                  receptive_field=args.receptive_field * args.framerate,
                                  num_detections=dataset_Test.num_detections, framerate=args.framerate).cuda()
    ...
    checkpoint = torch.load(os.path.join(
        f"{PATH_TO_AUTOMATIC_MODELS}/event_annotation/CALF/models", args.model_name, "model.pth.tar"),
        map_location=args.device)
    model.load_state_dict(checkpoint['state_dict'])

    return test(test_loader, model=model, model_name=args.model_name,
                save_predictions=args.save_predictions, confidence_threshold=args.confidence_threshold)
